...
# Restore: the reverse of backup — replaces DB_PATH and UPLOADS_DIR with the
# contents of an archive `backup` produced. Destructive; stop the server
# first. Prompts for confirmation unless --force is passed.
go run ./cmd/dsite restore ./backups/dsite-backup-20260101-120000.tar.gz
...
| `BACKUP_REMOTE_KEEP` | `30`           | Remote archives to retain; `0` keeps everything (unlike local, nothing else prunes the remote) |
...
**Backups**: `internal/backup` snapshots the DB via SQLite's `VACUUM INTO` (a consistent copy taken without blocking concurrent readers/writers under WAL) and bundles it with the whole `uploads/` directory into a single `.tar.gz` under `BACKUP_DIR`, rotating old local archives down to `BACKUP_KEEP`. Trigger it two ways: `dsite backup` (one-shot, e.g. from cron) or automatically inside the running server on `BACKUP_INTERVAL` (default 24h; first run fires after one interval, not immediately on startup, so redeploys don't each trigger a backup). A local copy is always written — `BACKUP_REMOTE` optionally also pushes the same archive off-site through the `backup.Remote` interface (`internal/backup/remote.go`), which requires both `Upload` and `Prune` (mirroring local rotation, but against `BACKUP_REMOTE_KEEP` — nothing else cleans up the remote, so without this every archive would accumulate there forever); the only backend implemented so far is `yadisk` (Yandex Disk's REST API, `internal/backup/yadisk.go`), which prunes by moving old archives to Yandex Disk's trash rather than a hard delete. Add an S3-compatible backend the same way if needed later — implement `Remote` and add a case to `backup.NewRemote`.

**Restore**: `dsite restore [--force] <archive.tar.gz>` (`internal/backup/restore.go`) is the reverse of `dsite backup` — it extracts an archive's `data.db` and `uploads/` over `DB_PATH`/`UPLOADS_DIR`, replacing whatever is currently there. This is destructive (prompts for confirmation unless `--force`) and must be run with the server stopped: it clears any stale `-wal`/`-shm` sidecars at `DB_PATH` before installing the restored file, and swapping the DB out from under a live WAL connection risks corrupting it. Extraction is staged into `<path>.restoring` next to the real path and only renamed into place once it succeeds, so an interrupted restore doesn't leave `DB_PATH`/`UPLOADS_DIR` half-replaced.
